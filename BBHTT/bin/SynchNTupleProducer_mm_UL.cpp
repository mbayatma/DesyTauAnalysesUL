...
int main(int argc, char * argv[]) {
  ...
  if (applyPUweight) {
    TFile * filePUdistribution_data = new TFile(TString(cmsswBase)+"/src/"+PileUpDataFile,"read"); 
    TFile * filePUdistribution_MC = new TFile (TString(cmsswBase)+"/src/"+PileUpMCFile, "read"); 
    TH1D * PU_data = (TH1D *)filePUdistribution_data->Get("pileup");
    TH1D * PU_mc = (TH1D *)filePUdistribution_MC->Get(PileUpMCHist);
    if (PU_mc == NULL) {
      std::cout << "Histogram " << pileUpMCHist << " is not present in pileup file" << std::endl;
      exit(-1);
    }
  ...
  // muon scale factors ---->
  // ************************
  const bool applyLeptonSF = cfg.get<bool>("ApplyLepSF");
  ...
  const string pfRecoilFileName   = cfg.get<string>("PFRecoilFileName");
  TString PFRecoilFileName(pfRecoilFileName);
  const string puppiRecoilFileName   = cfg.get<string>("PuppiRecoilFileName");
  TString PuppiRecoilFileName(puppiRecoilFileName);
  // HTT Met recoil corrections
  kit::RecoilCorrector recoilPFMetCorrector(PFRecoilFileName);
  kit::RecoilCorrector recoilPuppiMetCorrector(PuppiRecoilFileName);
  ...
  float ZweightMassMax = histZMassPtWeights->GetYaxis()->GetBinLowEdge(nBinsZweightMass+1);
  ...
  TFile * fileTagging = new TFile(TString(cmsswBase)+"/src/"+cfg.get<string>("BtagMCeffFile"));
  ...
  const vector<string> muonFilterNames   = cfg.get<vector<string>>("MuonFilterNames");
  ...
  // define DY and TTbar booleans
  bool isTop = (TStrName=="TTTo2L2Nu")||(TStrName=="TTToSemiLeptonic")||(TStrName=="TTToHadronic");
  bool isDY = (TStrName.Contains("DYJets"))||(TStrName.Contains("DY1Jets"))||(TStrName.Contains("DY2Jets"))||(TStrName.Contains("DY3Jets"))||(TStrName.Contains("DY4Jets"));
  bool isEmbedded = TStrName.Contains("Embed");
  ...
  // Histograms after final selection (used to derive mass,pt weight)
  TH2D * dimuonMassPtH = new TH2D("dimuonMassPtH","",200,0,1000,200,0,1000);
  TH2D * dimuonPtMassH = new TH2D("dimuonPtMassH","",200,0,1000,200,0,1000);
  ...
  TH1D * massExtendedSelH =  new TH1D("massExtendedSelH","",100,0,1000);
  ...
  TH1D * ptLeadingMuBSelH = new TH1D("ptLeadingMuBSelH","",100,0,200);
  TH1D * ptTrailingMuBSelH = new TH1D("ptTrailingMuBSelH","",100,0,200);
  TH1D * etaLeadingMuBSelH = new TH1D("etaLeadingMuBSelH","",50,-2.5,2.5);
  TH1D * etaTrailingMuBSelH = new TH1D("etaTrailingMuBSelH","",50,-2.5,2.5);
  
  TH1D * massBSelH = new TH1D("massBSelH","",200,0,200);
  TH1D * massExtendedBSelH =  new TH1D("massExtendedBSelH","",100,0,1000);
  ...
  TH1D * leadingBJetPtZSelH = new TH1D("leadingBJetPtZSelH","",80,20.,100.);
  TH1D * subleadingBJetPtZSelH = new TH1D("subleadingBJetPtZSelH","",80,20.,100.);
  TH1D * leadingBJetScoreZSelH = new TH1D("leadingBJetScoreZSelH","",100,-1,1);
  TH1D * subleadingBJetScoreZSelH = new TH1D("subleadingBJetScoreZSelH","",100,-1,1);
  TH1D * leadingBJetEtaZSelH = new TH1D("leadingBJetEtaZSelH","",50,-2.5,2.5);
  TH1D * subleadingBJetEtaZSelH = new TH1D("subleadingBJetEtaZSelH","",50,-2.5,2.5);
  ...
  for (int iF=0; iF<nTotalFiles; ++iF) {
	...
	*/
	...
	if (applyZMassPtReweighting&&isDY) {
	  if (genZMass>ZweightMassMax) genZMass=ZweightMassMax-0.5;
	...
	*/
	...
	if (applyTopPtReweighting&&isTop) {
	  float topptweight = genTools::topPtWeight_Run2(analysisTree, a_topPtWeight, b_topPtWeight, c_topPtWeight, max_pt_topPtWeight);
	  weight *= topptweight;
	}
      }
	...
	if (nBTagDiscriminant1==-1) {
	  std::cout << "Discriminator " << BTagDiscriminator1 << " is not found" << std::endl;
	  exit(-1);
	}
	if (nBTagDiscriminant2==-1) {
	  std::cout << "Discriminator " << BTagDiscriminator2 << " is not found" << std::endl;
	  exit(-1);
	}
      }
      if (BTagAlgorithm == "DeepFlavor") {
	if (nBTagDiscriminant3==-1) {
	  std::cout << "Discriminator " << BTagDiscriminator3 << " is not found" << std::endl;
	  exit(-1);
	}
      }
            
      //      std::cout << "NBTag = " << nBTagDiscriminant1 << " " << nBTagDiscriminant2 << " " << nBTagDiscriminant3 << std::endl;

      //      continue;
      //      std::cout << "Event " << analysisTree.event_nr << std::endl;
      unsigned int run = analysisTree.event_run;
      unsigned int lumiblock = analysisTree.event_luminosityblock;

      if ((isData || isEmbedded) && !isGoodLumi(run, lumiblock, json))
	continue;
	...
	allRuns.push_back(analysisTree.event_run);
      ...
      if (ApplyMetFilters && !passed_all_met_filters) continue;

      vector<unsigned int> nMuonFilters; nMuonFilters.clear();
      bool isMuonFilter = false;

      unsigned int nfilters = analysisTree.run_hltfilters->size();
      for (unsigned int i=0; i<nfilters; ++i) {
	TString HLTFilter(analysisTree.run_hltfilters->at(i));
	for (auto muonFilterName : muonFilterNames) {
	  TString MuonFilterName(muonFilterName);
	  if (HLTFilter==MuonFilterName) {
	    nMuonFilters.push_back(i);
	    isMuonFilter = true;
	    break;
	  }
	}
	...
	for (unsigned int iT=0; iT<analysisTree.trigobject_count; ++iT) {
	  ...
	  for (auto nMuonFilter : nMuonFilters) {
	...
	if (isPassedIdIso) {
      ...
      //      std::cout << "size = " << isoIdMuons.size() << std::endl;
	...
	for (unsigned int im1=0; im1<isoIdMuons.size(); ++im1) {
	  ...
	  if (isTriggerMatched &&  analysisTree.muon_pt[index1] > ptMuonCut && fabs(analysisTree.muon_eta[index1]) < etaMuonCut) {
		...
		if (analysisTree.pfjet_pt[jet]>jetPtHighCut) {
		  nJets30++;
		  if (fabs(analysisTree.pfjet_eta[jet])<bJetEtaCut) {
		    nJets30etaCut++;
		  }
		}
	  ...
	  for (unsigned int im2=im1+1; im2<isoIdMuons.size(); ++im2) {
	    ...
	    bool isTriggerMatched1 = isoIdMuonsIsTriggerMatched[im1] &&
	      ...
	      fabs(analysisTree.electron_eta[index1])<etaMuonCut;
            ...
            bool isTriggerMatched = isTriggerMatched1;
	    if (analysisTree.muon_pt[index2]>analysisTree.muon_pt[index1])
		...
		if (analysisTree.muon_pt[index1]>analysisTree.muon_pt[index2]) {
		  indx1 = index1;
		  indx2 = index2;
		  isoMu1 = isoIdMuonsIso[im1];
		  isoMu2 = isoIdMuonsIso[im2];
		}
		else {
		  indx2 = index1;
		  indx1 = index2;
		  isoMu2 = isoIdMuonsIso[im1];
		  isoMu1 = isoIdMuonsIso[im2];
		}
	      }
	    }
	  }
	...
	for (unsigned int jet=0; jet<analysisTree.pfjet_count; ++jet) {
	  ...
	  if (analysisTree.pfjet_pt[jet]>jetPtLowCut) {
	    ...
	    if (fabs(analysisTree.pfjet_eta[jet])<bJetEtaCut) {
	      ...
	      float bdiscr = analysisTree.pfjet_btag[jet][nBTagDiscriminant1];
	      if (BTagAlgorithm=="DeepCSV"||BTagAlgorithm=="DeepFlavour")
		bdiscr += analysisTree.pfjet_btag[jet][nBTagDiscriminant2];
		...
		if (jetPt>ptLeadingBJet) {
		  ...
		  scoreSubLeadingBJet = scoreLeadingBJet;
		  indexLeadingBJet = jet;
		  ptLeadingBJet = jetPt; 
		  scoreLeadingBJet = bdiscr;
		}
	...
	if ((!isData || isEmbedded) && applyLeptonSF) {
	  ...
	  */
	  ...
	  correctionWS->var("m_iso")->setVal(isoMu1);
	  ...
	  if (isEmbedded) {
	    ...
	    trigSF_mu1 = correctionWS->function("m_trg_ic_embed_ratio")->getVal();
	  }
	  else {
	    IdIsoSF_mu1 = correctionWS->function("m_idiso_ic_ratio")->getVal();
	    trigSF_mu1 = correctionWS->function("m_trg_ic_ratio")->getVal();
	  }

	  correctionWS->var("m_eta")->setVal(etaMu2);
          correctionWS->var("m_pt")->setVal(ptMu2);
          correctionWS->var("m_iso")->setVal(isoMu2);
	  ...
	  */
	  weight = weight*IdIsoSF_mu1*IdIsoSF_mu2*trackSF_mu1*trackSF_mu2*trigSF_mu1;
	...
	if (isDY && applyRecoilCorrections) {
	...
	if (massSel>dimuonMassCut) {
	  ...
	  dimuonPtMassH->Fill(dimuonPt,massSel,weight);
	  dimuonPtSelH->Fill(dimuonPt,weight);
	  ...
	  if (nJets20BTagMedium>=1) {
	    ...
	    ptTrailingMuBSelH->Fill(analysisTree.muon_pt[indx2],weight);
	    etaLeadingMuBSelH->Fill(analysisTree.muon_eta[indx1],weight);
	  ...
	  //	  std::cout << "njets = " << nJets30 << std::endl;
	  //	  std::cout << std::endl;
	  ...
	  if (massSel>70&&massSel<110) { // Z Region
	    ...
	    */
	    ...
	    if (indexLeadingBJet>=0) { 
	      ...
	      leadingBJetScoreZSelH->Fill(scoreLeadingBJet,weight);
	    }
	    if (indexSubLeadingBJet>=0) { 
	      subleadingBJetPtZSelH->Fill(ptSubLeadingBJet,weight);
	      double eta = analysisTree.pfjet_eta[indexSubLeadingBJet];
