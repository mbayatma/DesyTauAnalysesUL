...
int main(int argc, char ** argv) {
  ...
  if (argc!=4) {
    std::cout << "Usage : PlotSys [era=2016,2017,2018]  [Sample=TTTo2L2Nu, bbHToTauTau_yb2_M125, DYJetsToLL_M-50_amcatnlo] [channel=em,tt]" 
	      << std::endl;  
    exit(-1);
  ...
  TString cutsTrig = "((trg_muhigh_elow>0.5&&pt_2>24.0)||(trg_ehigh_mulow>0.5&&pt_1>24.0))";      
  TString cuts = cutsTrig + "&&iso_1<0.15&&iso_2<0.20&&extraelec_veto<0.5&&extramuon_veto<0.5&&dr_tt>0.3&&pt_1>15.&&pt_2>15.";

  if (channel=="tt") {
    cutsTrig = "(trg_doubletau>0.5)";
    cuts = cutsTrig + "&&extraelec_veto<0.5&&extramuon_veto<0.5&&dr_tt>0.5&&pt_1>40.&&pt_2>40.&&byVVLooseDeepTau2017v2p1VSe_1>0.5&&byVLooseDeepTau2017v2p1VSmu_1>0.5&&byVVLooseDeepTau2017v2p1VSe_2>0.5&&byVLooseDeepTau2017v2p1VSmu_2>0.5";
  }
...
void Plot(TFile * file,
  ...
  std::cout << "Nom  = " << histNominal->GetSumOfWeights() << " : " << histNominal->GetSum() << std::endl;
  std::cout << "Up   = " << histUp->GetSumOfWeights() << " : " <<  histUp->GetSum() << std::endl;
  std::cout << "Down = " << histDown->GetSumOfWeights() << " : " << histDown->GetSum() << std::endl;
