...
Double_t pol2(Double_t * x, Double_t * par) {
  ...
  if (x[0]>t)
    r = par[0]+par[1]*t+par[2]*t*t;//+par[3]*t*t*t;
...
void Plot_FR( 
  ...
  //    bins.push_back(0.5+0.3*double(i));
  ...
  TString var_to_save = Variable;
  TString xtitle = "p_{T}(#tau_{1}) [GeV]";
  TString ytitle = "Fake Factor";
  float xmin =                 0.5;
  float xmax =                 4.5;
  float yLower =                10;
  ...
  TString CutsSR("&&byMediumDeepTau2017v2p1VSjet_1>0.5&&byLooseDeepTau2017v2p1VSjet_2<0.5&&byVVVLooseDeepTau2017v2p1VSjet_2>0.5");
  TString CutsSB("&&byMediumDeepTau2017v2p1VSjet_1<0.5&&byVVVLooseDeepTau2017v2p1VSjet_1>0.5&&byLooseDeepTau2017v2p1VSjet_2<0.5&&byVVVLooseDeepTau2017v2p1VSjet_2>0.5");

  if (iso2) {
    CutsSR = TString("&&byMediumDeepTau2017v2p1VSjet_1>0.5&&byMediumDeepTau2017v2p1VSjet_2>0.5");
    CutsSB = TString("&&byMediumDeepTau2017v2p1VSjet_1<0.5&&byVVVLooseDeepTau2017v2p1VSjet_1>0.5&&byMediumDeepTau2017v2p1VSjet_2>0.5");
  }
  ...
  std::vector<TString> names = {"Data","VVL","TTL","W","ZTT","ZL"};
  ...
  for (auto name : names) {
    ...
    for (unsigned int j=0; j<Samples.size(); ++j) {
      ...
      TString sampleName = "tt-NOMINAL_ntuple_"+Samples.at(j)+"_"+era;
      ...
      std::cout << "   " << sampleName << "  entries = " << histSample->GetEntries() << "   yield = " << histSample->GetSumOfWeights() << std::endl;
  ...
  ratioH->GetYaxis()->SetTitle("Fake Factor");
  ratioH->GetXaxis()->SetTitle("p_{T}^{#tau1} (GeV)");
  ...
  TF1 * fitFunc = new TF1("fitFunc",pol2,40.,200.,4);
  ...
  TH1D * fr_histo = (TH1D*)ratioH->Clone("FR");

  ratioH->Fit("fitFunc","Q");
  delete dummy1;
    
  TH1D * hint = new TH1D("ff_"+era,"",160,40.,200.);   
  TH1D * hint_68 = new TH1D("ff_"+era+"_68","",160,40.,200.);   
  ...
  //  ratioH->SetBinContent(nbins,update);
  ...
  TString os_suffix("ss");
  if (OS)
    os_suffix = "os";

  if (nbtags!="") {
    TFile * fileOutput = new TFile("ff_nbtags_"+nbtags+"_"+os_suffix+"_"+era+".root","recreate");
    fileOutput->cd("");
    ratioH->Write("ratioH");
    fileOutput->Close();
  }
  ...
  leg->SetHeader("njets"+njets+"_dm"+dm);
  leg->Draw();
  ...
  if (iso2 && !OS)
    canv->Print("ff_figures/"+var_to_save+suffix+"_njets"+njets+"_dm"+dm+"_ss_iso2_"+era+".png");
  if (!iso2 && !OS)
    canv->Print("ff_figures/"+var_to_save+suffix+"_njets"+njets+"_dm"+dm+"_ss_antiiso2_"+era+".png");
  if (!iso2 && OS) 
    canv->Print("ff_figures/"+var_to_save+suffix+"_njets"+njets+"_dm"+dm+"_os_antiiso2_"+era+".png");

  if (nbtags=="") {
    TFile * fileFF = new TFile("ff_njets"+njets+"_dm"+dm+"_era"+era+".root","recreate");
    fileFF->cd("");
    fileFF->Write("fr");
    fileFF->Close();
  }
